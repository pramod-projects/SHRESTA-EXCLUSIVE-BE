...
public final class CustomerPaymentTransactionsMigration {

    private CustomerPaymentTransactionsMigration() {}

    public static TransitionPlan transitionPlan() {
        return TransitionPlan.forTable("customer_payment_transactions")
            .transition(List.of(-1), 0, List.of(
                ...
                """,
                "CREATE UNIQUE INDEX uq_customer_payment_tx_provider_payment ON customer_payment_transactions (provider, payment_id)",
                "CREATE INDEX idx_customer_payment_tx_order ON customer_payment_transactions (order_number, last_webhook_received_at DESC)",
                "CREATE INDEX idx_customer_payment_tx_status ON customer_payment_transactions (payment_status, last_webhook_received_at DESC)"
            ))
            .build();
    }
}
