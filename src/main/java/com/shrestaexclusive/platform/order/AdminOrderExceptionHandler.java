...
class AdminOrderExceptionHandler {
    ...
    ApiResponse<Void> invalidRequest(MethodArgumentNotValidException exception) {
        return ApiResponse.failed("INVALID_ADMIN_ORDER_REQUEST", "Enter valid order status update details.", traceId());
    }
    ...
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    ApiResponse<Void> conflict(CustomerOrderPlacementException exception) {
        return ApiResponse.failed("ADMIN_ORDER_STATUS_UPDATE_FAILED", exception.getMessage(), traceId());
    }
