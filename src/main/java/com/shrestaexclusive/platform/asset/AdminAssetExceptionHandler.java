...
class AdminAssetExceptionHandler {
    ...
    public ApiResponse<Void> unauthorized(StorefrontAdminUnauthorizedException exception) {
        return ApiResponse.failed("ADMIN_UNAUTHORIZED", exception.getMessage(), traceId());
    }

    @ExceptionHandler(AssetNotFoundException.class)
    @ResponseStatus(HttpStatus.NOT_FOUND)
    public ApiResponse<Void> notFound(AssetNotFoundException exception) {
        return ApiResponse.failed("ASSET_NOT_FOUND", exception.getMessage(), traceId());
    }

    @ExceptionHandler({IllegalArgumentException.class, MethodArgumentNotValidException.class})
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    public ApiResponse<Void> invalid(Exception exception) {
    ...
    @ResponseStatus(HttpStatus.SERVICE_UNAVAILABLE)
    public ApiResponse<Void> storageUnavailable(MediaStorageException exception) {
        return ApiResponse.failed("MEDIA_STORAGE_UNAVAILABLE", exception.getMessage(), traceId());
    }

    @ExceptionHandler(IllegalStateException.class)
    @ResponseStatus(HttpStatus.UNPROCESSABLE_ENTITY)
    public ApiResponse<Void> processingFailed(IllegalStateException exception) {
