...
@RestControllerAdvice(assignableTypes = AdminCategoryController.class)
class AdminCategoryExceptionHandler {
    ...
    ApiResponse<Void> invalidRequest(RuntimeException exception) {
        return ApiResponse.failed("INVALID_CATEGORY_REQUEST", exception.getMessage(), traceId());
    }
    ...
    @ResponseStatus(HttpStatus.CONFLICT)
    ApiResponse<Void> dataConflict(DataAccessException exception) {
        return ApiResponse.failed("CATEGORY_DATA_CONFLICT", "Category update conflicts with existing catalog data", traceId());
    }
