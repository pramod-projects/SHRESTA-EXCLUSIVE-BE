...
class CustomerAuthExceptionHandler {
    ...
    ApiResponse<Void> invalidRequest(MethodArgumentNotValidException exception) {
        ...
        if ("requestOtp".equals(methodName)) {
            return ApiResponse.failed("INVALID_CUSTOMER_IDENTITY", "Enter a valid email address or 10 digit Indian mobile number.", traceId());
        }
    ...
    @ExceptionHandler(CustomerOtpRequestException.class)
    @ResponseStatus(HttpStatus.TOO_MANY_REQUESTS)
    ApiResponse<Void> otpRequestLimited(CustomerOtpRequestException exception) {
        return ApiResponse.failed("CUSTOMER_OTP_REQUEST_LIMITED", exception.getMessage(), traceId());
    }
