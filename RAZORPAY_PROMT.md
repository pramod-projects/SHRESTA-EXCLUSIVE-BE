...
RAZORPAY_KEY_ID: rzp_test_TNZxCWuxxc5qV0
RAZORPAY_KEY_SECRET: ALuJuiEafzsHZRB0X4PZkJCv
...
Add these values to the matching backend runtime file (`.env.dev`, `.env.uat`, or `.env.prod`):
...
Runtime `.env.<mode>` files are gitignored. Commit only `.env.<mode>.example` templates.
...
2. Create the matching `.env.<mode>` file from its `.env.<mode>.example` template
