...
Future infrastructure changes retain Cloudflare R2 as the canonical media store and require measured operational justification.
...
`GET /api/v1/storefront/home` returns the backend-owned customer home dataset from PostgreSQL. It is assembled by `StorefrontHomeService` from `storefront_home_sections`, `storefront_home_items`, and READY `media_assets`. The response includes one canonical Cloudflare custom-domain URL and source dimensions per media item. Public product cards require a READY primary image; the admin read keeps image-less newly created products visible for attachment.
...
`StorefrontMediaUrlBuilder` maps immutable object keys through required `MEDIA_PUBLIC_BASE_URL`. Production and UAT use configured Cloudflare custom media domains. Immutable UUID keys do not use query-string versioning.

DEV uses MinIO through `docker-compose.dev.yml` as an R2 protocol-compatible target. UAT and PROD use Cloudflare R2 through `R2_ACCOUNT_ID`, `R2_ENDPOINT`, `R2_BUCKET_NAME`, server-only credentials, and `MEDIA_PUBLIC_BASE_URL`. The browser uploads one canonical object directly using a short-lived presigned PUT URL.
...
Admin asset APIs live under `/api/v1/admin/assets`. Upload authorization validates ownership, type, canonical dimensions, byte size, duration, and product limits; completion verifies R2 object size, type, and media identity before READY. Search/detail, metadata, archive/delete, and bulk category assignment operate on canonical metadata. Primary-image approval links the new READY asset key and archives the old key through explicit lifecycle handling. `GET /api/v1/admin/assets/storefront-unreferenced` lists READY admin-managed media with no storefront primary, video, or active gallery reference so admins can relink idle assets instead of uploading duplicates. The `storefront-product-media-link` change request type links such an asset to a product's PRIMARY, GALLERY_1..GALLERY_4, or VIDEO slot: submission rejects a submission when a pending request for the same product targets a different slot (same-slot resubmission upserts), the asset's content kind must match the slot (`video/*` for VIDEO, `image/*` for PRIMARY/GALLERY), and eligibility plus reassignment retag `product_sku`/`media_type` only for unreferenced assets or assets already owned by the item with a matching media type. Approval swaps the slot reference and archives the replaced asset when unreferenced. Permanent asset deletion is blocked while any storefront primary/video or active gallery reference exists and hard-deletes inactive gallery ghost rows before removing the `media_assets` row. The abandoned display-upload sweep and product-reservation expiry skip assets referenced by a PENDING_REVIEW `storefront-product-media-link` payload `assetKey`.

`media_assets` carries category, product, tags, alt text, SEO fields, canonical dimensions, object key, content type, byte size, status, uploader, expiry, and ETag metadata. It never stores media binaries or derivative rows.

Historical migration `V16__s3_compatible_asset_urls_and_replacements.sql` normalized legacy localhost media paths. Current runtime behavior is defined by the canonical R2 metadata migration and `MEDIA_PUBLIC_BASE_URL`; historical migrations remain immutable.
...
- Full-stack development mode: run `./up dev`; the canonical backend controller owns Compose dependencies, backend/frontend readiness, environment tracking, and localhost-only tunnel policy.
